...
-- Single query → all 16 checks with PASS/FAIL. Paste the result back.
-- Numeric checks are normalized with round(...)::bigint so trailing decimal
-- precision can never cause a false FAIL. No financial data is read-modified.
-- ══════════════════════════════════════════════════════════════
with chk as (
  select 1 n,'Unit count' k,(select count(*)::text from units) actual,'42' expected
  union all select 2,'Historical realized profit (sold)',(select round(coalesce(sum(keuntungan_bersih::numeric),0))::bigint::text from units where status='terjual'),'131800000'
  union all select 3,'Capital allocation total',(select round(coalesce(sum(amount),0))::bigint::text from capital_allocations),'847500000'
  ...
  union all select 5,'Reserve closing balance',(select round(closing_reserve)::bigint::text from v_reserve_reconcile),'1700000'
  union all select 6,'Inventory economic cost (active)',(select round(coalesce(sum(economic_cost),0))::bigint::text from v_inventory),'98000000'
  union all select 7,'P&L Realized Unit Profit',(select round(realized_unit_profit)::bigint::text from v_pnl),'131800000'
  union all select 8,'P&L Net Operating Profit',(select round(net_operating_profit)::bigint::text from v_pnl),'131100000'
  union all select 9,'Opening cash (unverified baseline)',(select value from app_config where key='opening_cash'),'0'
  union all select 10,'Distribution total (recorded, preserved)',(select round(coalesce(sum(profit_share),0))::bigint::text from capital_allocations),'171909600'
  ...
  union all select 12,'#35/37/38 kas preserved',(select string_agg(round(kas_bisnis::numeric)::bigint::text,'/' order by id) from units where id in(35,37,38)),'800000/800000/100000'
  union all select 13,'#37 unresolved variance',(select round(coalesce(sum(amount),0))::bigint::text from settlement_variances where unit_id=37 and status='unresolved'),'50000'
